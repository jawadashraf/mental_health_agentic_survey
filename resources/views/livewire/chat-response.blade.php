@if($message['type'] === 'stream')
    <div class="w-3/4 space-y-0.5 has-[.stream:empty]:hidden">
        ...
        <div class="bg-slate-200 rounded-xl rounded-tl-none px-3 py-1.5 text-sm">
            <div wire:stream="stream-{{ $this->getId()  }}">{{ count($messages) == 2 ? $greetings : $response }}</div>
        </div>
    </div>
@elseif($message['type'] === 'question')
    <div class="mb-2 font-medium">{{ $message['content']['question'] }}</div>

    @if($message['content']['type'] === 'radio')
        <div class="space-y-2">
            @foreach($message['content']['options'] as $option)
    ...
    @elseif($message['content']['type'] === 'text')
        <div class="mt-2">
